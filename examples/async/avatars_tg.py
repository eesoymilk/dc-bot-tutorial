...
async def fetch_avatar(user_id: int) -> str:
    ...
    async with asyncio.TaskGroup() as tg:
        user = tg.create_task(fetch_user(user_id))

    # simulate the time it takes to download the image or whatever
    # However, here we are just returning the url of the image
    await asyncio.sleep(1)
    return user.result().avatar_url


async def main():
    async with asyncio.TaskGroup() as tg:
        tasks = [
            tg.create_task(fetch_avatar(1)),
            tg.create_task(fetch_avatar(2))
        ]

    for task in tasks:
        webbrowser.open(task.result())
